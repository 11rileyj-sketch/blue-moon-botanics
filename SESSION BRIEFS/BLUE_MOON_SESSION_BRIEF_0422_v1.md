...
>
> 1. **Cache stale fix** — `fetch_beta_user_record` TTL (600s) may be returning stale `Onboarded` data on login, causing a redirect loop back to auth. Fix: reduce TTL or add `fetch_beta_user_record.clear()` after `upsert_user()`. Verify login works cleanly end to end after fix.
> 2. **Profile sidebar** — implementation brief ready (`BRIEF_PROFILE_SIDEBAR.md`). Hand to Code. Test on mobile and desktop.
> 3. **Make.com batch session** — intake is broken, requires a dedicated Make.com session. Do not mix into app.py work. See Make.com section for full queue.
> 4. **Scientific name capitalization** — display-layer fix: `scientific_name[0].upper() + scientific_name[1:]` in `render_result_card`. One line, low risk.
> 5. **"Great news" message persistence** — session state artifact, persists across tab switches. Cleanup candidate, low priority.
...
**APR 22 2026 — Session 16 (Deploy session — Railway auth + onboarding):** Green mode. CAVU. Full deploy session — no vision work.

**Completed:**

- Auth0 working on Railway end to end — login, collection, logout all verified
- Diagnosed root cause: Railway has no secrets.toml support — Streamlit auth system needs TOML config, not raw env vars
- Built `startup.sh` pattern — generates `.streamlit/secrets.toml` from Railway env vars at deploy time. Permanent solution for Railway/Streamlit auth deployments.
- Added `AUTH0_REDIRECT_URI` and `COOKIE_SECRET` as Railway env vars
- Updated `railway.toml` start command to `bash startup.sh`
- Added `https://blue-moon-botanics-production.up.railway.app/oauth2callback` to Auth0 Allowed Callback URLs and Allowed Logout URLs
- Fixed `fetch_collection` formula — changed `FIND()` to `{{Beta User}} = '{beta_user}'` for linked record field compatibility
- Nuked old plant data — fresh start under "Justin Riley" identity
- Mobile verified — login, intake, collection all working on phone
- Onboarding flow built and deployed — new vs. returning user gate, display name editable, optional ZIP_Code field, June's line: "Optional. Helps me give location-aware care advice. I won't do anything weird with it."
- `Onboarded` checkbox + `ZIP_Code` fields added to Beta Users Airtable table
- `get_location()` refactored — reads from Airtable `ZIP_Code` field instead of local `user_settings.json` (works on Railway now)
- `display_name` now reads `st.session_state.get("display_name")` first — supports user-edited names from onboarding
- Profile sidebar implementation brief written (`BRIEF_PROFILE_SIDEBAR.md`)
- Favorite plant — confirmed backlog, flagged as good weekly push candidate

**Decided:**

- Zip field named `ZIP_Code` — preserves leading zeros, handles zip+4 format
- Avatar/profile photo field deferred — no upload UI yet, sidebar uses initial-based circle avatar for now
- `st.logout()` takes no arguments — button label handled separately with `st.button("Sign out")` wrapping it
- "Beta Users" table name stays as-is for now — renaming risks breaking field references. Conceptually it's becoming a user profile store.

**Open issues going into next session:**

1. Cache stale on login — `fetch_beta_user_record` TTL 600s may return stale `Onboarded = False` after first login, causing redirect loop. Fix: reduce TTL or add cache clear after `upsert_user()`
2. Profile sidebar — brief ready, not yet built
3. Make.com batch session — intake broken, full queue unchanged
4. Scientific name capitalization — `scientific_name[0].upper() + scientific_name[1:]` display-layer fix
5. "Great news" message persistence — session state artifact, low priority cleanup
...
### 1. Cache stale on login — onboarding redirect loop

- **Symptom:** After first login, app briefly loads then kicks back to auth screen.
- **Cause:** `fetch_beta_user_record` TTL is 600s — may return stale record with `Onboarded = False` even after checkbox is set in Airtable.
- **Fix:** Add `fetch_beta_user_record.clear()` after `upsert_user()` call, or reduce TTL to 60s.
...
- [x] Auth0 authentication — working on Railway end to end
- [x] startup.sh — generates secrets.toml from Railway env vars at deploy time
- [x] fetch_collection formula — fixed for linked record field (FIND → equality check)
- [x] Mobile verified — login, intake, collection all working
- [x] Onboarding flow — new vs. returning user gate, display name + ZIP_Code, June's zip line
- [x] get_location() — reads from Airtable ZIP_Code field instead of local file
- [x] display_name — reads session_state first, supports user-edited names
- [x] secrets.toml
...
- startup.sh ← generates .streamlit/secrets.toml from Railway env vars at boot
...
- `AUTH0_REDIRECT_URI` — Railway env var: `https://blue-moon-botanics-production.up.railway.app/oauth2callback`
- `COOKIE_SECRET` — Railway env var: any long random string, must match secrets.toml
